...
const ChatForm = memo(function ChatForm({
   ...
   *  (possibly empty) overrides stop `ask` from vacuuming manual skill picks
   *  the user has staged in the composer for their NEXT message. */
  ...
  /** Chip "Edit message" restore: skill picks merge back into their
   *  compose-time atom (the chips above the textarea re-render them). */
  const restoreComposerContext = useRecoilCallback(
    ({ set }) =>
      (context?: QueuedMessageContext) => {
        const { manualSkills } = context ?? {};
  ...
  const steering = useSteering({
    ...
    conversation,
    isSubmitting,
    // No ask-answer-mode (HITL Q&A) in this fork — the composer is never
    // borrowed for an in-run question, so during-run steer/queue always applies.
   ...
   *  merges the steer's attachments into the composer's file map and its
   *  skill picks into their atom, so restoring over staged context would glue
