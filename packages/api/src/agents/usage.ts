...
/**
 * Cache-read tokens across provider shapes. Mirrors `getCacheCreationTokens`
 * so the read and write sides never drift apart.
 *
 * `prompt_tokens_details.cached_tokens` is the OpenAI-compatible spelling: a
 * proxy speaking the OpenAI wire format reports hits only there, and this fork
 * routes most models through one (OpenRouter). Without it every cache hit on
 * that path bills at the full input rate.
 */
function getCacheReadTokens(usage: UsageMetadata): number {
  return (
