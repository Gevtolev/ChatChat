...
describe('recordCollectedUsage', () => {
  ...
  /**
   * A proxy speaking the OpenAI wire format reports cache hits only as
   * `prompt_tokens_details.cached_tokens` — neither the langchain nor the
   * Anthropic spelling appears. This fork routes most models through one
   * (OpenRouter), so without this shape every hit on that path would fall
   * through to `spendTokens` and bill the cached prefix at the full input
   * rate.
   */
  describe('cache token handling - OpenAI-compatible proxy format', () => {
    it('bills prompt_tokens_details.cached_tokens as a cache read', async () => {
      /** 140 = 100 fresh + 40 read; the proxy folds cache into input_tokens. */
      const collectedUsage: UsageMetadata[] = [
        {
          input_tokens: 140,
          output_tokens: 50,
      ...
      expect(mockSpendStructuredTokens).toHaveBeenCalledWith(
        ...
        {
          promptTokens: { input: 100, write: 0, read: 40 },
          completionTokens: 50,
        },
      );
      expect(result?.input_tokens).toBe(140);
    ...
    it('still bills the whole prompt at the input rate when no cache is reported', async () => {
      const collectedUsage: UsageMetadata[] = [
        {
          input_tokens: 140,
          output_tokens: 50,
      ...
      expect(mockSpendTokens).toHaveBeenCalledWith(
        expect.objectContaining({ model: 'deepseek-chat' }),
        { promptTokens: 140, completionTokens: 50 },
      );
    });
  });
