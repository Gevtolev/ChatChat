...
/**
 * `AbortSignal.any` is supported at runtime by our target Node versions, but its
 * type declarations only ship with TypeScript >=5.6's `lib.dom.d.ts`. This repo is
 * pinned to TypeScript 5.3.x, so it's accessed through a narrow local cast instead
 * of bumping the compiler version repo-wide.
 */
...
function buildSignal(runSignal?: AbortSignal, hookSignal?: AbortSignal): AbortSignal {
  ...
  const abortSignalWithAny = AbortSignal as AbortSignalWithAny;
  if (signals.length > 0 && typeof abortSignalWithAny.any === 'function') {
    return abortSignalWithAny.any([...signals, timeout]);
  }
  return timeout;
}
