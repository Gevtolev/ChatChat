...
> **预估**: 6-10 小时
> **优先级**: 高 —— 但**不是紧急**。当前处于测试阶段，`balance.enabled` 为假、`DISABLE_BILLING_GATING=true`，没有任何用户在被扣费，因此费率错误目前只损害**度量**而非用户。它的价值在于：开启计费之前必须先让仪表准，否则定价决策建立在错误数据上
...
`librechat.yaml` 的 `modelSpecs` 共 28 个对用户开放的模型，其中 **16 个计费费率错误**。

### 1.0 当前影响范围：只是度量，还不是钱

必须先说清楚，否则下文的金额会被误读。生产处于**测试阶段**：`balance.enabled` 未配置、`DISABLE_BILLING_GATING=true`，因此

- **没有任何用户余额被扣减** —— 费率只写进 `Transaction` 记录
- 生产至今全部消耗合计约 **$0.40**，无实际损失

所以本 spec 修的是**仪表刻度**，不是正在流血的伤口。下文所有"净亏"“多扣”均指**开启计费之后**才会成立的金额。

它仍然重要，因为顺序是硬的：**定价决策依赖成本数据，成本数据依赖费率准确**。带着 20 倍误差的数据去标定积分发放量，等于把错误放大进商业模型。
...
**现在（测试阶段，未开计费）**：唯一后果是成本看板会给出错误数字，且**方向随用户的模型选择而变** —— 少扣的模型让总数偏低、多扣的让它偏高，无法用统一系数校正。这会直接污染积分发放量的标定。

**开启计费之后**：

- **少扣**：真实成本高于记录成本，差额不出现在任何账目里，告警不会触发
- **多扣**：用户月度积分被过快消耗，提前撞到"额度用尽"，表现为留存损失
